	...
	public void acheterProduit(String nomAcheteur) {
		if(!controlAcheterProduit.verifierIdentite(nomAcheteur)) {
			...
			if(vendeurs.length==0) {
				...
				int quantiteRestante=controlAcheterProduit.modifierQuantite(choixQuantite,vendeurs[choixCommercant]);
				if(quantiteRestante==0) {
					System.out.println(nomAcheteur+" veut acheter "+choixQuantite+" "+produit+", malheureusement il n'y en a plus !");
				}
				else if(quantiteRestante<choixQuantite) {
					System.out.println(nomAcheteur+" veut acheter "+choixQuantite+" "+produit+", malheureusement "+ vendeurs[choixCommercant]+" n'en a plus que "+quantiteRestante+". "+nomAcheteur+" achète tout le stock de "+vendeurs[choixCommercant]);
					
					;
				}else {
					System.out.println(nomAcheteur+" achète "+choixQuantite+" "+produit+" à"+ vendeurs[choixCommercant]+".");
				}
